...
class GetScoreBoardTests(APITestCase):
    ...
    def test_should_return_200(self):
        url = reverse('batch_scoreboard', args=[1])
    ...
    def test_should_return_data_if_score_updated(self):
        ...
        self.assertEqual(res_body[0].get("email"), 'Batch1+user1@example.com')
        self.assertEqual(res_body[0].get("score"), 100)
        self.check_cost_expectation(100, 200)

    def test_for_second_request_for_same_batch_should_fetch_from_cache(self):
        url = reverse('batch_scoreboard', args=[1])
        self.update_score(1, 1, 100)
    ...
    def test_should_set_batch_level_cache(self):
        # Update Cache For 1st Batch
        url = reverse('batch_scoreboard', args=[1])
        self.update_score(1, 1, 100)
        response_1_batch_1 = self.make_new_request(url)
        
        # 1st Request Batch 1 Expectation
        res_1_batch_1_body = json.loads(response_1_batch_1.content)
        self.assertEqual(response_1_batch_1.status_code, 200)
        self.assertEqual(len(res_1_batch_1_body), 1)
        self.check_cost_expectation(100, 200)

        # Update Cache For 2nd Batch
        url = reverse('batch_scoreboard', args=[2])
        self.update_score(101, 2, 100)
        self.update_score(102, 2, 1000)
        self.update_score(103, 2, 10000)
        response_1_batch_2 = self.make_new_request(url)
        
        # 1st Request Batch 2 Expectation
        res_1_batch_2_body = json.loads(response_1_batch_2.content)
        self.assertEqual(len(res_1_batch_2_body), 3)
        self.check_cost_expectation(100, 200)
        
        # 2nd Request Batch 1
        url = reverse('batch_scoreboard', args=[1])
        response_2_batch_1 = self.make_new_request(url)

        # 2nd Request Batch 1 Expectation
        res_2_batch_1_body = json.loads(response_2_batch_1.content)
        self.assertEqual(len(res_2_batch_1_body), 1)
        self.check_cost_expectation(1, 50)
        self.assertEqual(res_2_batch_1_body, res_1_batch_1_body)

        # 2nd Request Batch 2
        url = reverse('batch_scoreboard', args=[2])
        response_2_batch_2 = self.make_new_request(url)

        # 2nd Request Batch 2 Expectation
        res_2_batch_2_body = json.loads(response_2_batch_2.content)
        self.assertEqual(len(res_2_batch_2_body), 3)
        self.check_cost_expectation(1, 50)
        self.assertEqual(res_2_batch_2_body, res_1_batch_2_body)
    ...
    def update_score(self, user_id, batch_id, score):
        bu = BatchUser.objects.filter(batch_id=batch_id, user_id=user_id).first()
        bu.score = score
        bu.save()
